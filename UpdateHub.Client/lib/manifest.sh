...
# Отбирает из манифеста строки, которые сервер не примет.
#
# md5sum помечает ведущим обратным слэшем строки, где имя файла содержит
# перевод строки или сам обратный слэш. Такие имена не передать построчным
# форматом, и сервер их отклоняет. Молчать об этом нельзя: файл просто
# никогда не обновится, и никто не поймёт почему.
