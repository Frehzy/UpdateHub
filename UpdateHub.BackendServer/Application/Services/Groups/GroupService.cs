...
namespace UpdateHub.BackendServer.Application.Services.Groups;
    ...
    ILogger<GroupService> logger) : IGroupService
{
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Группа удаляется по-настоящему, а не помечается неактивной. Пометка
    /// заводила в тупик: название остаётся занятым уникальным индексом, список
    /// групп невидимую запись не показывает, и завести «Бухгалтерию» заново
    /// становится невозможно без внятной причины.
    /// <para>
    /// Терять при этом нечего: связи настроены как раз под удаление —
    /// компьютеры остаются без группы, а выданные на неё права исчезают.
    /// В истории обращений группы не участвуют.
    /// </para>
    /// </remarks>
    public async Task DeleteAsync(string groupId, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("Группа {Name} удалена", group.Name);
    }
