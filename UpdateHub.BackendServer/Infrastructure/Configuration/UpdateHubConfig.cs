...
public class UpdateHubConfig
{
    ...
    /// <summary>
    /// Сколько неудачных попыток входа допускается для одной учётной записи.
    /// Ноль и меньше отключают ограничение.
    /// </summary>
    /// <remarks>
    /// Считаются только неудачи, и только по имени учётной записи: удачный вход
    /// обнуляет счёт, поэтому работающая машина ограничителя не встретит,
    /// сколько бы машин ни приходило с одного адреса. Прежний вариант считал
    /// обращения по адресу и любые, включая удачные, — за криптомаршрутизатором
    /// это перекрывало обновления всему парку, и его пришлось снять.
    /// </remarks>
    public int LoginFailureLimit { get; set; } = 10;

    /// <summary>
    /// На сколько минут закрывается вход после исчерпания попыток.
    /// </summary>
    /// <remarks>
    /// Пяти минут достаточно, чтобы перебор стал бессмысленно медленным,
    /// и мало, чтобы помешать человеку, вспомнившему пароль.
    /// </remarks>
